...
<div class="d-sm-flex align-items-center justify-content-between mb-4 ">
    <div>
        <ol class="breadcrumb fs-sm mb-1 ">
            ...
            <li class="breadcrumb-item active" aria-current="page">Edit Skoring</li>
        </ol>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <select class="form-select @error('operator') is-invalid @enderror" 
                        ...
                        <option value="<" {{ old('operator', $skoring->operator) == '<' ? 'selected' : '' }}>&lt;</option>
                        <option value="<=" {{ old('operator', $skoring->operator) == '<=' ? 'selected' : '' }}>&lt;=</option>
                        <option value="=" {{ old('operator', $skoring->operator) == '=' ? 'selected' : '' }}>=</option>
                        <option value=">=" {{ old('operator', $skoring->operator) == '>=' ? 'selected' : '' }}>&gt;=</option>
                        <option value=">" {{ old('operator', $skoring->operator) == '>' ? 'selected' : '' }}>&gt;</option>
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control @error('harga_satuan') is-invalid @enderror"
                               id="harga_satuan" name="harga_satuan" value="{{ old('harga_satuan', $skoring->harga_satuan) }}" required>
                    </div>
                    @error('harga_satuan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <select class="form-select @error('kategori') is-invalid @enderror" 
                        ...
                        <option value="">Pilih Data...</option>
                        <option value="1" {{ old('kategori', $skoring->kategori) == '1' ? 'selected' : '' }}>Belanja Modal</option>
                        <option value="2" {{ old('kategori', $skoring->kategori) == '2' ? 'selected' : '' }}>Belanja Barang</option>
                    </select>
                    @error('kategori')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-4">
                    <select class="form-select @error('umur_ekonomis') is-invalid @enderror" 
                        ...
                        @foreach(Barang::$umurEkonomis as $key => $value)
                            <option value="{{ $key }}" {{ old('umur_ekonomis', $skoring->umur_ekonomis) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('umur_ekonomis')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-4">
                    <select class="form-select @error('jenis_barang') is-invalid @enderror" 
                        ...
                        @foreach(Barang::$jenisBarang as $key => $value)
                            <option value="{{ $key }}" {{ old('jenis_barang', $skoring->jenis_barang) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('jenis_barang')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-4">
                    <select class="form-select @error('sifat_barang') is-invalid @enderror" 
                        ...
                        @foreach(Barang::$sifatBarang as $key => $value)
                            <option value="{{ $key }}" {{ old('sifat_barang', $skoring->sifat_barang) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('sifat_barang')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-4">
                    <select class="form-select @error('keterangan') is-invalid @enderror" 
                        ...
                        @foreach(Barang::$keterangan as $key => $value)
                            <option value="{{ $key }}" {{ old('keterangan', $skoring->keterangan) == $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                    @error('keterangan')
                        <div class="invalid-feedback">{{ $message }}</div>
